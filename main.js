...
document.getElementById("form1").addEventListener("submit", (e) => {
  ...
  let budgetInputsValues = document.querySelectorAll(
    "#budgetSource, #budgetAmount"
  );
  budgetInputsValues.forEach((input) => {
    input.value = "";
  });
  ...
  budgetSpan.textContent =
    parseInt(budgetAmount) + parseInt(budgetSpan.textContent);
...
document.getElementById("form2").addEventListener("submit", (e) => {
  ...
  let expensesInputsValues = document.querySelectorAll(
    "#expensesSource, #expensesAmount"
  );
  expensesInputsValues.forEach((input) => {
    input.value = "";
  });
  ...
  expensesSpan.textContent =
    parseInt(expensesAmount) + parseInt(expensesSpan.textContent);
  totalSpan.textContent = budgetSpan.textContent - expensesSpan.textContent;

  myChart.data.datasets[0].data.splice(
    1,
    1,
    parseInt(expensesSpan.textContent)
  );
  myChart.update();
    ...
    datasets: [
      {
        ...
        borderColor: ["rgb(0, 0, 255)", "rgb(255, 0, 0)", "rgb(0, 128, 0)"],
        borderWidth: 1,
      },
    ],
  },
